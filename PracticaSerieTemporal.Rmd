...
```{r}
close <- Dataset[,c(5)]
close.ts = ts(close, start = 0, frequency = 5)
```
...
Vamos a comprobar el Ljung Box Test para ver la bondad del modelo creado. El Ljung Box Test indica que no se rechaza la hipótesis nula por lo que los datos siguen una distribución de ruido blanco y el modelo no es fiable a la hora de realizar predicciones.
...
De todas formas realizaremos la predicción de los 20 próximos días.

```{r}
library(astsa)
arima_model <- sarima(close.ts, p = 0, d = 1, q = 0)
result <- sarima.for(close.ts, n.ahead=20, 0,1,0)
```
